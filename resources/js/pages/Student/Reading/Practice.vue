...
<template>
    <div class="space-y-6 max-w-3xl">
        <div class="flex items-center gap-3">
            <Link :href="route('student.reading.index')" class="text-gray-400 hover:text-gray-600">
        ...
        <div v-if="questions.length" class="space-y-3">
            <Link
                ...
                :key="q.id"
                :href="route('student.reading.question', q.id)"
                class="card p-4 flex items-center gap-4 hover:shadow-md transition-shadow group"
            >
                <div class="w-8 h-8 rounded-full bg-cyan-100 text-cyan-700 font-bold text-sm flex items-center justify-center flex-shrink-0">{{ i + 1 }}</div>
                <div class="flex-1 min-w-0">
                    <p class="font-medium text-gray-900 truncate">{{ q.title }}</p>
                    <span class="text-xs capitalize text-gray-400">{{ q.difficulty }}</span>
                </div>
                <span class="text-cyan-600 text-sm font-medium group-hover:underline flex-shrink-0">Practice →</span>
            </Link>
        </div>
