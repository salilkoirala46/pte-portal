...
<template>
    <div class="space-y-6 max-w-3xl mx-auto">
        <div class="card p-5 border-amber-200 bg-amber-50">
            <h2 class="font-semibold text-amber-800">Select Missing Word</h2>
            <p class="text-sm text-amber-700 mt-1">The last word(s) of the recording are replaced by a beep. Select the correct option.</p>
        </div>

        <AudioPlayer v-if="question.audio_url" :src="question.audio_url" :play-once="true" label="Recording — last word replaced by beep (plays once)"/>
        ...
        <div class="card p-6 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                ...
                >
                    <input type="radio" :value="opt.id" v-model="selected" class="accent-primary-600"/>
